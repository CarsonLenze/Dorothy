    ...
    run: async (button, args) => {
        ...
        if (button.componentType == 'SELECT_MENU') {
            ...
            for (const key of Object.keys(query)) {
                row.addComponents(
                    new MessageButton()
                        .setCustomId(`player-${button.user.id}-${button.player.id}-${key}`)
                        .setLabel(key.split('_').map(s => s == 'op' ? 'OP' : s[0].toUpperCase() + s.substring(1)).join(' '))
                        .setStyle('SECONDARY')
                        .setDisabled(args[2] == key)
                );
            }
        } else {
            const request = await api.get(`/other/${args[2]}/${args[1]}/data`).catch(() => { /* ERR */ });
            if (!request) {
                const error = new MessageEmbed()
                    .addField(`${DESIGN.redx} No Data Found`, `There was no data found for that user.`)
                    .setColor(DESIGN.red);

                return button.reply({ embeds: [error] });
            }
            query[args[2]] = request.data.body;
        }

        const server = args[2], data = query[server]
        switch (server) {
            case 'op_factions':
        ...
        const embed = new MessageEmbed()
            .setAuthor(button.player ? { name: button.player.tag, iconURL: button.player.icon } : button.message.embeds[0].author)
        ...
        if (button.componentType == 'SELECT_MENU') return button.editReply({ embeds: [embed], components: [row] });

        button.message.components[0].components.forEach(obj => {
            obj.disabled = false;
            let name = obj.customId.split('-')[3];
            if (name == server) obj.disabled = true;
        });
        await button.update({ embeds: [embed], components: button.message.components });
    },
};